...
html += '<td class="text-nowrap text-center">';
    ...
    window.editarRol = function(id) {
        $.get('{{ url("/api/admin/roles") }}/' + id, function(data) {
...
function asignarPermisos(id) {
    ...
    if (permisosData.length === 0) {
        $.get('{{ url("/api/admin/permisos") }}', function(data) {
...
function guardarPermisos() {
    ...
    $.ajax({
        url: '{{ url("/api/admin/roles") }}/' + id + '/permisos',
        method: 'POST',
        data: JSON.stringify({ permisos: permisos }),
