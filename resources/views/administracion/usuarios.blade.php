...
function cargarUsuarios(pagina = 1) {
    ...
    $.get('{{ url("/api/admin/usuarios") }}?' + params.toString(), function(data) {
...
function cargarRoles() {
    $.get('{{ url("/api/admin/roles") }}', function(data) {
...
function verDetalle(id) {
    $.get('{{ url("/api/admin/usuario") }}/' + id, function(data) {
        if (data.user) {
...
function guardarRoles() {
    ...
    $.ajax({
        url: '{{ url("/api/admin/usuario") }}/' + id + '/roles',
        method: 'POST',
...
function toggleActivo(id, nombre) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ url("/api/admin/usuario") }}/' + id + '/toggle-activo',
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                },
